...
**Bold terms** are defined below; look them up here for the full meaning.

## Invocation

Two choices, trading different costs:

- A **model-invoked** capability is an OpenCode skill with a **description**, so the agent can fire it autonomously and other workflows can reach it (you can still load it explicitly too). It contributes to **context load** because the description sits in the window every turn. Write a model-facing description with rich trigger phrasing ("Use when the user wants…, mentions…").
- A **user-invoked** capability is an OpenCode command under `commands/`, reachable only when the user types its slash command. Its `description` is human-facing: a one-line summary with trigger lists stripped. It spends **cognitive load** because the user must remember it exists, but adds no model-invoked skill description.
