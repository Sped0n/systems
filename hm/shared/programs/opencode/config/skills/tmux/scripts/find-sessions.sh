...
List tmux sessions on a socket. Defaults to the opencode socket path.

Options:
  -L, --socket-name  tmux socket name (passed to tmux -L)
  -S, --socket-path  tmux socket path (passed to tmux -S)
  -A, --all          scan all sockets under OPENCODE_TMUX_SOCKET_DIR
...
tmux_args=()
socket_label="default opencode socket"
...
if [[ -n "$socket_name" ]]; then
  ...
  socket_path="$socket_dir/opencode.sock"
  tmux_args+=(-S "$socket_path")
  socket_label="socket path '$socket_path'"
fi
