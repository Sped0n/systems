...
PANE_ID="$("${TMUX[@]}" new-session -d -P -F '#{pane_id}' -s "$SESSION" -n shell)"
"${TMUX[@]}" send-keys -t "$PANE_ID" -- 'command' Enter
"${TMUX[@]}" capture-pane -p -J -t "$PANE_ID" -S -200
```

`-f /dev/null` keeps Pi-owned servers independent of the user's tmux configuration; set `PI_TMUX_CONFIG` only when the task needs a deliberate isolated config. The config is read when the server first starts, so replace an existing server to apply a different config.
