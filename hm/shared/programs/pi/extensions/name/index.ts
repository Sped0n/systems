  ...
  if (response.stopReason === "error" || response.stopReason === "aborted") {
    throw new Error(
      response.errorMessage ||
        `Session title generation ${response.stopReason}`,
    );
  }
...
export function handleNameEditorSubmission(
  editor: Pick<
    EditorComponent,
    "addToHistory" | "getExpandedText" | "getText" | "setText"
  >,
  data: string,
...
async function nameSession(
  ...
  if (requestedName) {
    ...
    ctx.ui.notify(
      `Session named: ${pi.getSessionName() ?? requestedName}`,
      "info",
    );
    return;
  }
  ...
  if (!ctx.isIdle()) {
    ctx.ui.notify(
      "Wait for the current response to finish before generating a session name",
      "warning",
    );
    return;
  }
  ...
  try {
    ...
    ctx.ui.notify(
      `Could not name session: ${error instanceof Error ? error.message : String(error)}`,
      "error",
    );
  }
}
