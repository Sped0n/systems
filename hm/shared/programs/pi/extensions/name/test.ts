...
test("buildConversationText includes only user and assistant text", () => {
	const text = buildConversationText([
		{ type: "message", message: { role: "user", content: [{ type: "text", text: "Fix naming" }] } },
		{ type: "message", message: { role: "assistant", content: [{ type: "text", text: "Implemented it" }, { type: "toolCall", name: "edit" }] } },
...
test("cleanGeneratedTitle removes thinking and bounds OpenCode-style output", () => {
	assert.equal(cleanGeneratedTitle("<think>draft</think>\nUseful session title\nignored"), "Useful session title");
