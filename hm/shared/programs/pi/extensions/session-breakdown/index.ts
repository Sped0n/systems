 ...
 * /session-breakdown
 *
 * Interactive TUI that analyzes sessions under the configured Pi directory and shows
...
interface LoaderWithSetMessage {
...
function setBorderedLoaderMessage(loader: BorderedLoader, message: string) {
  ...
  const inner = (loader as unknown as { loader?: LoaderWithSetMessage }).loader;
  if (inner) {
...
function extractProviderModelAndUsage(obj: unknown): {
  ...
  const entry = isRecord(obj) ? obj : {};
  const msg = isRecord(entry.message) ? entry.message : {};
  return {
    api: entry.api ?? msg.api,
...
function extractCostTotal(usage: unknown): number {
  ...
  const t = isRecord(c) ? c.total : undefined;
...
function extractTokensTotal(usage: unknown): number {
  ...
  total =
    ...
    readFiniteNumber(usage.tokens) ||
    readFiniteNumber(usage.tokenCount) ||
    readFiniteNumber(usage.token_count);
  if (total > 0) return total;

  // nested tokens object
  const tokens = isRecord(usage.tokens) ? usage.tokens : {};
  total =
    readFiniteNumber(tokens.total) ||
    readFiniteNumber(tokens.totalTokens) ||
    readFiniteNumber(tokens.total_tokens);
  if (total > 0) return total;

  // sum of parts
  const a =
    readFiniteNumber(usage.promptTokens) ||
    readFiniteNumber(usage.prompt_tokens) ||
...
function dayMixedColor(
  ...
  if (view === "dow") {
    ...
    const sByModel = groupProviders
      ? day.groupedSessionsByModel
      : day.sessionsByModel;
    const mByModel = groupProviders
      ? day.groupedMessagesByModel
      : day.messagesByModel;
    const tByModel = groupProviders
...
class BreakdownComponent implements Component {
  ...
  render(width: number): string[] {
    ...
    lines.push(
      truncateToWidth(
        dim(
          `←/→ range · ↑/↓ view · tab metric · p providers (${this.splitProviders ? "split" : "grouped"}) · q to close`,
        ),
        width,
      ),
    );
...
export default function sessionBreakdownExtension(pi: ExtensionAPI): void {
  pi.registerCommand("session-breakdown", {
    description:
      "Interactive breakdown of recent Pi session usage (sessions/messages/tokens + cost by model)",
