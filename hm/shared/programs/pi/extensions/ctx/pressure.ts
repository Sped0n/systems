...
const MILESTONES = [0.70, 0.85, 0.95] as const;
const ADVICE = [
...
export function cycleId(entries: readonly SessionEntry[]): string {
  return entries.findLast((entry) => entry.type === "compaction")?.id ?? "initial";
}

export function reminderKey(entries: readonly SessionEntry[], window: number, reserve: number): string {
  return `${cycleId(entries)}:${window}:${reserve}`;
}

/** Jumps emit only the highest newly crossed milestone; state comes from this branch. */
export function nextReminder(entries: readonly SessionEntry[], tokens: number, window: number, reserve: number) {
  const budget = window - reserve;
  if (!Number.isFinite(tokens) || tokens < 0 || !Number.isFinite(budget) || budget <= 0) return;
  const level = MILESTONES.findLastIndex((ratio) => tokens >= Math.ceil(budget * ratio));
  if (level < 0) return;
  const key = reminderKey(entries, window, reserve);
  const delivered = entries.reduce((highest, entry) => {
    if (entry.type !== "custom_message" || entry.customType !== REMINDER_TYPE) return highest;
    const details = entry.details as { key?: unknown; level?: unknown } | undefined;
    return details?.key === key && typeof details.level === "number" ? Math.max(highest, details.level) : highest;
  }, -1);
  if (level <= delivered) return;
  return {
    key,
    level,
    text: `[Context pressure: ${Math.round(MILESTONES[level] * 100)}% milestone of the pre-compaction budget.] `
