...
function renderEntry(entry: SessionEntry): RecallEntry | undefined {
  ...
  const message = entry.type === "message" ? entry.message : { ...entry, role: "custom" as const };
  if (message.role === "bashExecution") {
    if (message.excludeFromContext) return;
    return { id: entry.id, role: message.role, text: `$ ${message.command}\n${message.output}` };
  }
  if (message.role === "compactionSummary" || message.role === "branchSummary") {
    return { id: entry.id, role: message.role, text: message.summary };
  }
  if (message.role === "toolResult" && ["recall", "vcc_recall"].includes(message.toolName)) return;
  const text = typeof message.content === "string" ? message.content : message.content.map((part) => {
    ...
    if (part.type === "toolCall") {
      return ["recall", "vcc_recall"].includes(part.name) ? "" : `${part.name} ${JSON.stringify(part.arguments)}`;
    }
    return "";
  }).filter(Boolean).join("\n");
  if (!text.trim()) return;
...
export function registerRecall(pi: ExtensionAPI) {
  pi.registerCommand("recall", {
    description: "Recover session context, optionally focused on a question or request",
    handler: async (args) => {
      if (!pi.getActiveTools().includes("recall")) {
        throw new Error("/recall requires the recall tool to be active.");
      }
      const request = args.trim() || "Recover the current task's goals, constraints, decisions, progress, blockers, and next steps.";
      pi.sendUserMessage(
  ...
  pi.registerTool({
    ...
    async execute(_id, params, signal, _onUpdate, ctx) {
      ...
      if (action === "read") {
        ...
        if (!entry) throw new Error(`Entry ${JSON.stringify(target)} has no recallable text in scope '${scope}'. Search first and copy a returned read request.`);
        if (offset >= entry.text.length) throw new Error(`offset must be less than ${entry.text.length}.`);
        const end = Math.min(offset + 12000, entry.text.length);
        const next = end < entry.text.length ? { action, target, offset: end, scope } : null;
        return result(
      ...
      const next = end < hits.length ? { action, target, offset: end, scope } : null;
      const reads = selected.map((entry) => ({ action: "read" as const, target: entry.id, offset: 0, scope }));
      return result(
        `Scope: ${scope}; matches ${offset + 1}-${end}/${hits.length}.\n\n`
          + selected.map((entry, index) => `[${entry.id}] ${entry.role}\n${entry.snippet}\nRead: recall(${JSON.stringify(reads[index])})`).join("\n\n")
          + (next ? `\n\nContinue: recall(${JSON.stringify(next)})` : ""),
