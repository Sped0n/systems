...
test("runtime dependency versions match development dependencies", async () => {
  ...
  const runtimePackage = JSON.parse(
    await readFile(new URL("../../runtime/package.json", import.meta.url), "utf8"),
  ) as { dependencies: Record<string, string> };

  const developmentVersions = Object.fromEntries(
    Object.keys(runtimePackage.dependencies).map((name) => [name, rootPackage.dependencies[name]]),
  );
...
test("canonicalizePath resolves a symlinked write parent", async () => {
  ...
  assert.equal(await canonicalizePath("link/new/file", root), path.join(await realpath(actual), "new", "file"));
});
...
test("runtime rules append after file policy and dispose independently", async () => {
  ...
  try {
    ...
    release();
    release();
    assert.equal(await toolCall({ toolName: "bash", input: { command: "npm test" } }, ctx), undefined);
  } finally {
    release();
...
test("Git inspection policy allows Git and rg reads but denies hazardous forms", async () => {
  for (const bash of [
    ...
    assert.equal((await commandDecision(GIT_INSPECTION_BASH_POLICY, bash, process.cwd())).action, "allow", bash);
  }
  for (const bash of [
    ...
    assert.equal((await commandDecision(GIT_INSPECTION_BASH_POLICY, bash, process.cwd())).action, "deny", bash);
  }
});
test("Bash rules use wildcards and last match wins", async () => {
  ...
  assert.equal((await commandDecision(policy, "git status --short", process.cwd())).action, "allow");
  assert.equal((await commandDecision(policy, "git push origin main", process.cwd())).action, "deny");
  assert.equal((await commandDecision(policy, "npm test", process.cwd())).action, "deny");
});
...
test("deny rules return custom reasons without matching prefixed commands", async () => {
  ...
  assert.deepEqual(await commandDecision(policy, "pip install requests", process.cwd()), {
    action: "deny",
    reason: "Use `uv pip` instead.",
  });
  assert.deepEqual(await commandDecision(policy, "pip3 install requests", process.cwd()), {
    action: "deny",
    reason: "Use `uv pip` instead.",
  });
  assert.equal((await commandDecision(policy, "uv pip install requests", process.cwd())).action, "allow");
});
...
test("every command in a shell chain must be allowed", async () => {
  ...
  assert.deepEqual(await parseBashCommands("git status && git diff | git apply --check"), ["git status", "git diff", "git apply --check"]);
  assert.equal((await commandDecision(policy, "git status && npm test", process.cwd())).action, "deny");
  assert.equal((await commandDecision(policy, "git status; rm -rf build", process.cwd())).action, "deny");
  assert.equal((await commandDecision(policy, "git log --format='a;b'", process.cwd())).action, "allow");
});
...
test("complex shell substitutions evaluate nested commands", async () => {
  ...
  assert.equal((await commandDecision(policy, "printf '%s' $(cat secret)", process.cwd())).action, "deny");
  assert.equal((await commandDecision(policy, "printf \"`cat secret`\"", process.cwd())).action, "deny");
  assert.equal((await commandDecision(policy, "diff <(first) <(second)", process.cwd())).action, "deny");
});
...
test("valid Bash without an executable command is allowed", async () => {
  ...
  assert.equal((await commandDecision(policy, "NAME=value", process.cwd())).action, "allow");
});
...
test("a final catch-all allow bypasses parsing complex shell syntax", async () => {
  ...
  const heredoc = "node <<'JS'\nconsole.log(process.cwd())\nJS";
  assert.equal((await commandDecision(policy, heredoc, process.cwd())).action, "allow");
});
...
test("narrower allows after catch-all allow still bypass parsing", async () => {
  ...
  assert.equal((await commandDecision(policy, "printf '%s' $(cat secret)", process.cwd())).action, "allow");
});
...
test("a restrictive rule after catch-all allow evaluates nested commands", async () => {
  ...
  assert.equal((await commandDecision(policy, "printf '%s' $(rm secret)", process.cwd())).action, "deny");
});
...
test("last matching path rule wins and no match denies", async () => {
  ...
  assert.equal((await pathDecision(policy, "read", "secret/approved", root)).action, "allow");
  assert.equal((await pathDecision(policy, "write", "new-file", root)).action, "deny");
});
...
test("trusted project rules append after global rules", async () => {
  ...
  await writeFile(path.join(global, "interceptor.json"), JSON.stringify({
    rules: [{ path: "$CWD/**", operations: ["write"], action: "deny" }],
  }));
  await writeFile(path.join(root, ".pi", "interceptor.json"), JSON.stringify({
    rules: [{ path: "$CWD/generated/**", operations: ["write"], action: "allow" }],
  }));

  const trusted = await loadPolicies({ cwd: root, projectTrusted: true, globalConfigDir: global });
  assert.ok("rules" in trusted);
  if ("rules" in trusted) assert.equal((await pathDecision(trusted.rules, "write", "generated/file", root)).action, "allow");

  const untrusted = await loadPolicies({ cwd: root, projectTrusted: false, globalConfigDir: global });
  assert.ok("rules" in untrusted);
  if ("rules" in untrusted) assert.equal((await pathDecision(untrusted.rules, "write", "generated/file", root)).action, "deny");
});

test("malformed eligible project policy fails closed", async () => {
  const root = await tempRoot();
  await mkdir(path.join(root, ".pi"));
  await writeFile(path.join(root, ".pi", "interceptor.json"), JSON.stringify({ read: { allow: [] } }));
  const globalConfigDir = path.join(root, "missing-global");
  assert.ok("error" in await loadPolicies({ cwd: root, projectTrusted: true, globalConfigDir }));
  assert.ok("rules" in await loadPolicies({ cwd: root, projectTrusted: false, globalConfigDir }));
});
...
test("dangling policy symlinks fail closed instead of looking absent", async () => {
  ...
  const global = path.join(root, "global");
  await mkdir(global);
  await symlink(path.join(root, "missing-policy.json"), path.join(global, "interceptor.json"));
  assert.ok("error" in await loadPolicies({ cwd: root, projectTrusted: false, globalConfigDir: global }));
});
...
test("placeholders and symlinks retain sensitive lexical matching without allowing escapes", async () => {
  ...
  assert.equal((await pathDecision(policy, "write", "link/new-file", root)).action, "deny");
  assert.equal((await pathDecision(policy, "read", "link/new-file", root)).action, "deny");
  assert.equal((await pathDecision(policy, "read", path.join(process.env.HOME!, "does-not-exist", "key"), root)).action, "deny");
  const canonicalPolicy = parsePolicy({ rules: [{ path: "$CWD/actual/**", operations: ["write"], action: "allow" }] });
  assert.equal((await pathDecision(canonicalPolicy, "write", "alias/new-file", root)).action, "allow");
});
