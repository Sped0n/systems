...
test("buildReviewPrompt preserves free-form scope and focus instructions", () => {
  const instructions =
    "Review main...HEAD under src/auth, focusing on authorization.";
  assert.match(
    buildReviewPrompt(instructions, ""),
    new RegExp(instructions.replaceAll(".", "\\."), "u"),
  );
});

test("filterReviewAutocompleteItems exposes end-review only during review", () => {
  const items = [{ value: "review" }, { value: "end-review" }];
  assert.deepEqual(filterReviewAutocompleteItems(items, false), [
    { value: "review" },
  ]);
...
test("review preserves the selected model, restricts tools, and restores the previous tool set", async () => {
  ...
  const pi = {
    ...
    on: (
      name: string,
      handler: (_event: unknown, ctx: ExtensionCommandContext) => void,
    ...
    setModel: () => {
      throw new Error("Review must retain the selected model");
    },
...
test("findLatestReviewReport returns the latest assistant text", () => {
  ...
  assert.equal(
    findLatestReviewReport([textMessage("user", "no report yet")]),
    "",
  );
  assert.equal(
    findLatestReviewReport([
