...
function safeSliceEnd(text: string, length: number): string {
	...
	if (start > 0 && start < text.length && /[\uDC00-\uDFFF]/u.test(text[start])) {
		start++;
	}
...
export function boundReviewBrief(brief: string): string {
	...
	const tailLength =
		REVIEW_BRIEF_CHARACTERS_MAX - REVIEW_BRIEF_HEAD_CHARACTERS - REVIEW_BRIEF_OMISSION.length;
	return [
...
function getReviewConversationBrief(ctx: ExtensionContext): string {
	try {
		...
		return buildReviewConversationBrief(context.messages.filter((message) => "role" in message) as Message[]);
	} catch {
		return "";
	}
}

export function buildReviewPrompt(instructions: string, conversationBrief: string): string {
	const requestedInstructions = instructions.trim() || "Review all staged, unstaged, and untracked changes.";
...
function setReviewWidget(ctx: ExtensionContext, active: boolean): void {
	...
	ctx.ui.setWidget(REVIEW_WIDGET_KEY, (_tui, theme) => {
		return new Text(theme.fg("dim", "Review session · /end-review to return"), 0, 0);
	});
}
...
async function prepareReview(
	...
	const workTree = await pi.exec("git", ["rev-parse", "--is-inside-work-tree"], {
		cwd: ctx.cwd,
		timeout: 5_000,
	});
	...
	if (!instructions.trim()) {
		const status = await pi.exec("git", ["status", "--porcelain=v1", "--untracked-files=all"], {
			cwd: ctx.cwd,
			timeout: 5_000,
		});
		if (status.code !== 0) {
			return { ok: false, error: status.stderr.trim() || "Unable to inspect uncommitted changes." };
		}
...
export function filterReviewAutocompleteItems<T extends { value: string }>(
	...
	return reviewActive ? items : items.filter((item) => item.value !== "end-review");
}
...
export default function reviewExtension(pi: ExtensionAPI): void {
	...
	const installReviewAutocomplete = (ctx: ExtensionContext) => {
		...
		ctx.ui.addAutocompleteProvider((current) => ({
			...
			async getSuggestions(lines, cursorLine, cursorCol, options) {
				...
				return {
					...suggestions,
					items: filterReviewAutocompleteItems(suggestions.items, reviewOriginId !== undefined),
				};
			},
	...
	pi.registerMessageRenderer(REVIEW_RESULT_TYPE, (message, { outputPad }, theme) => {
		const content =
	...
	pi.registerCommand("review", {
		description: "Start an isolated review branch (usage: /review [instructions])",
		handler: async (args, ctx) => {
			...
			if (!preflight.ok) {
				ctx.ui.notify(
					preflight.error,
					preflight.error === "No uncommitted changes to review." ? "info" : "error",
				);
				return;
			}
	...
	pi.registerCommand("end-review", {
		...
		handler: async (_args, ctx) => {
			...
			if (!ctx.isIdle()) {
				ctx.ui.notify("Interrupt the active review before using /end-review.", "warning");
				return;
			}
			...
			try {
				...
				ctx.ui.notify(`Failed to end review: ${error instanceof Error ? error.message : String(error)}`, "error");
				return;
			} finally {
				navigationInProgress = false;
			}
